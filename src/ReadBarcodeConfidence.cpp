 ...
 */
bool InitialRuntimeSettingsWithAPI(CBarcodeReader& reader, int conThreshold)
{
	...
	runtimeSettings.expectedBarcodesCount = 100;
	runtimeSettings.barcodeFormatIds = BarcodeFormat::BF_ONED;
	runtimeSettings.furtherModes.textFilterModes[0] = TextFilterMode::TFM_SKIP;
	runtimeSettings.localizationModes[0] = LocalizationMode::LM_CONNECTED_BLOCKS;
	runtimeSettings.localizationModes[1] = LocalizationMode::LM_LINES;
	runtimeSettings.localizationModes[2] = LocalizationMode::LM_SKIP;
	runtimeSettings.localizationModes[3] = LocalizationMode::LM_SKIP;
	runtimeSettings.deblurLevel = 2;
	runtimeSettings.minResultConfidence = conThreshold;
...
bool InitialRuntimeSettingsWithFile(CBarcodeReader& reader)
{
	...
	file = file.substr(0, pos + 1) + "resource/ReadBarcodeConfidence.json";
...
bool GetConfidenceThreshold(int& conThreshold)
{
	...
	while (1)
	{
		printf("\r\nStep 1: Input confidence threshold [0-100]: (0 means no limitation on the confidence. )\r\n>> ");
#if defined(_WIN32) || defined(_WIN64)
		...
		if (iLen > 0)
		{
			...
			int confidence = atoi(pszBuffer);
			if (confidence >= 0 && confidence <= 100)
			{
				conThreshold = confidence;
				break;
			}
		}
...
#if defined(_WIN32) || defined(_WIN64)
		sprintf_s(pszTemp, 4096, "    Confidence: %d\r\n", (*paryResult->results[iIndex]->results)->confidence);
#else
		snprintf(pszTemp, 4096, "    Confidence: %d\r\n", (*paryResult->results[iIndex]->results)->confidence);
#endif
		printf(pszTemp);
...
int main(int argc, const char* argv[])
{
	...
	printf("*************************************************\r\n");
	printf("The sample image path is resource/ReadBarcodeConfidence.jpg\r\n");
	printf("The template file path is resource/ReadBarcodeConfidence.json\r\n");
	...
	GetExeDirectory(file);
	size_t pos = file.find_last_of('/');
	file = file.substr(0, pos + 1) + "resource/ReadBarcodeConfidence.jpg";

	int iRet = -1;
	int confThreshold = 0; // 0 means no limitation on the confidence.
	bool bExit = false;
	const char* pszTemplateName = NULL;

	while (1)
	{
		bExit = GetConfidenceThreshold(confThreshold);
		if (bExit)
			break;

		//Initialize runtimesettings by using PublicRuntimeSettings structure
		bExit = InitialRuntimeSettingsWithAPI(reader, confThreshold);

		//Initialize runtimesettings by using a template file
		//In this way, you can change the value of "MinResultConfidence" in resource/ReadBarcodeConfidence.json
