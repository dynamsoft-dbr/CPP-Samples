 ...
 */
bool InitialRuntimeSettingsWithAPI(CBarcodeReader& reader, bool enableTextFilter)
{
	...
	runtimeSettings.furtherModes.textFilterModes[0] = enableTextFilter ? TextFilterMode::TFM_GENERAL_CONTOUR : TextFilterMode::TFM_SKIP;
	...
	if (enableTextFilter)
	{
		iRet = reader.SetModeArgument("TextFilterModes", 0, "Sensitivity", "9", errorMessage, 256);
	}
	if (iRet != DBR_OK)
		return true;
	return false;
...
bool InitialRuntimeSettingsWithFile(CBarcodeReader& reader)
{
	...
	file = file.substr(0, pos + 1) + "resource/TextFilter.json";
...
bool GetEnableTextFilter(bool& tof)
{
	...
	while (1)
	{
		printf("\r\n>> Step 1: Enbale text filter parser? [y/n]:\r\n");
#if defined(_WIN32) || defined(_WIN64)
		...
		if (iLen > 0)
		{
			...
			if (pszBuffer[0] == 'Y' || pszBuffer[0] == 'y')
			{
				tof = true;
				break;
			}
			else if(pszBuffer[0] == 'N' || pszBuffer[0] == 'n')
			{
				tof = false;
				break;
			}
...
int main(int argc, const char* argv[])
{
	...
	printf("*************************************************\r\n");
	printf("The sample image path is resource/TextFilter.png\r\n");
	printf("The template file path is resource/TextFilter.json\r\n");
	...
	GetExeDirectory(file);
	size_t pos = file.find_last_of('/');
	file = file.substr(0, pos + 1) + "resource/TextFilter.pdf";


	while (1)
	{
		bExit = GetEnableTextFilter(enableTextFilter);
		if (bExit)
			break;

		//Initialize runtimesettings by using PublicRuntimeSettings structure
		bExit = InitialRuntimeSettingsWithAPI(reader, enableTextFilter);
