 ...
 */
bool InitialRuntimeSettingsWithAPI(CBarcodeReader& reader, int deblurLevel)
{
	...
	if (iRet != DBR_OK)
		return true;
	runtimeSettings.deblurLevel = deblurLevel;
...
bool GetDeblurLevel(int& deblurLevel)
{
	...
	while (1)
	{
		printf("\r\nPlease input deblur level (hight/low)[H/L]:\r\n>> ");
#if defined(_WIN32) || defined(_WIN64)
		...
		if (iLen > 0)
		{
			...
			{
				if (pszBuffer[0] == 'h' || pszBuffer[0] == 'H')
				{
					deblurLevel = 9;
					break;
				}
				else if (pszBuffer[0] == 'l' || pszBuffer[0] == 'L')
				{
					deblurLevel = 3;
					break;
				}
		...
		printf("Please input a valid deblur level.\r\n");
	}
	return bExit;
}
...
int main(int argc, const char* argv[])
{
	...
	printf("*************************************************\r\n");
	printf("The sample image path is resource/ReadBarcodeDeblur.png\r\n");
	printf("The template file path is resource/ReadBarcodeDeblur.json\r\n");
	...
	file = file.substr(0, pos + 1) + "resource/ReadBarcodeDeblur.png";

	int deblurLevel = 0;
	while (1)
	{
		bExit = GetDeblurLevel(deblurLevel);
		if (bExit)
			break;
		//Initialize runtimesettings by using PublicRuntimeSettings structure
		bExit = InitialRuntimeSettingsWithAPI(reader, deblurLevel);
