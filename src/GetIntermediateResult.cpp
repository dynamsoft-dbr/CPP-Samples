...
bool CreateIntermediateReusltsFolder()
{
	int iRet = 0;
	const char folder[] = "IntermediateResults";
#if defined(_WIN64) || defined(_WIN32)
	...
	{
		iRet = mkdir(folder, S_IRUSR | S_IWUSR);
		if (iRet != 0)
			return true;
	}
#endif
...
bool InitialRuntimeSettingsWithAPI(CBarcodeReader& reader)
{
	...
	runtimeSettings.intermediateResultSavingMode = IntermediateResultSavingMode::IRSM_FILESYSTEM;
	runtimeSettings.intermediateResultTypes = IntermediateResultType::IRT_BINARIZED_IMAGE;

	string file;
	GetExeDirectory(file);
	size_t pos = file.find_last_of('/');
	file = file.substr(0, pos + 1) + "IntermediateResults";

	iRet = reader.UpdateRuntimeSettings(&runtimeSettings, errorMessage, 256);
	iRet |= reader.SetModeArgument("IntermediateResultSavingMode", 0, "FolderPath", file.c_str(), errorMessage, 256);
	if (iRet != DBR_OK)
		return true;
	return false;
}
...
bool InitialRuntimeSettingsWithFile(CBarcodeReader& reader)
{
	...
	GetExeDirectory(file);
	size_t pos = file.find_last_of('/');
	file = file.substr(0, pos + 1) + "resource/GetIntermediateResult.json";
...
int main(int argc, const char* argv[])
{
	...
	printf("*************************************************\r\n");
	printf("You can find the intermediate results in \"IntermediateResults/\"\r\n");
	printf("Hints: Please input 'Q' or 'q' to quit the application.\r\n");

	CBarcodeReader reader;
	reader.InitLicense("Put your license key here");

	bExit = CreateIntermediateReusltsFolder();
	if (bExit)
		printf("create intermediate results folder failed.\r\n");
