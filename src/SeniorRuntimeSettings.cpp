...
bool InitialRuntimeSettingsWithAPI(CBarcodeReader& reader)
{
	...
	if (iRet != DBR_OK)
		return true;
	runtimeSettings.binarizationModes[0] = BinarizationMode::BM_LOCAL_BLOCK;
	iRet = reader.UpdateRuntimeSettings(&runtimeSettings, errorMessage, 256);
	iRet |= reader.SetModeArgument("BinarizationModes", 0, "BlockSizeX", "20", errorMessage, 256);
	iRet |= reader.SetModeArgument("BinarizationModes", 0, "BlockSizeY", "20", errorMessage, 256);
	iRet |= reader.SetModeArgument("BinarizationModes", 0, "EnableFillBinaryVacancy", "1", errorMessage, 256);
	iRet |= reader.SetModeArgument("BinarizationModes", 0, "ThreshValueCoefficient", "10", errorMessage, 256);
	if (iRet != DBR_OK)
		return true;
	return false;
}
...
bool InitialRuntimeSettingsWithFile(CBarcodeReader& reader)
{
	...
	file = file.substr(0, pos + 1) + "resource/SeniorRuntimeSettings.json";
